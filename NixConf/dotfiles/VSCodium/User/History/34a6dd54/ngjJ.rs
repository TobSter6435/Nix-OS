...
const API_KEY = std::env("API_KEY");

#[tauri::command]
fn get_weather(city: String) -> Result<Value, String> {
    let client = Client::new();
    let response = client.get(format!("https://api.openweathermap.org/data/2.5/weather?q={}&appid={}&units=metric", city, API_KEY)).send().unwrap();
